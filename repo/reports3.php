...
<head>
    ...
    $sql = mysql_query("SELECT product_name, avg(sell_price) as total FROM offer off, product_line pl WHERE pl.production_id = off.production_id and company_id = $profile_id Group By product_name");
    
    $rows = array();
    $table= array();
    $table['cols']=array(
        array('label'=> 'Nombre de Producto','type' => 'string'),
        array('label'=> 'Promedio de Precio de Producto', 'type' => 'number')
    );
