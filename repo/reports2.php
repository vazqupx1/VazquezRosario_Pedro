...
<head>
    ...
    $sql = mysql_query("SELECT product_name, offer_quantity * sell_price as total FROM offer off, product_line pl WHERE pl.production_id = off.production_id and profile_id = $profile_id Group By product_name");
    
    $rows = array();
    $table= array();
    $table['cols']=array(
        array('label'=> 'Nombre de Producto','type' => 'string'),
        array('label'=> 'Total Vendido', 'type' => 'number')
    );
